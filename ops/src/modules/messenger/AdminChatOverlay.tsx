...
const CHAT_CLIENT_ID_STORAGE_KEY = 'ops.chat.clientId.v1';
...
const CHAT_READ_RECEIPT_FAST_REFRESH_MS = 1400;
...
const AdminChatOverlay: React.FC<AdminChatOverlayProps> = ({
  ...
  const normalizedRouteConversationId = useMemo(() => normalizeText(routeConversationId), [routeConversationId]);
  ...
  useEffect(() => {
    ...
    if (embedded) {
      // Messenger route active: always visible as full in-page chat.
      setOpen(true);
    } else if (wasEmbedded && hideLauncher) {
      // Leaving embedded ops route should not leave drawer/floating UI open.
      setOpen(false);
    }
  ...
  useEffect(() => {
    if (!embedded || !normalizedRouteConversationId) return;
    setActiveConversationId((prev) => (prev === normalizedRouteConversationId ? prev : normalizedRouteConversationId));
    setOpen(true);
  }, [embedded, normalizedRouteConversationId]);

  useEffect(() => {
    if (!hideLauncher || embedded || !open) return;
    // Ops mode should not keep any detached/floating chat UI open outside messenger routes.
    setOpen(false);
  }, [hideLauncher, embedded, open]);
  ...
  const callOverlayBottom = hideLauncher || embedded ? 86 : 96;
  ...
  const activeDirectConversationHasPendingReadReceipts = useMemo(() => {
    if (!activeConversation || activeConversation.type !== 'direct') return false;
    const myUserId = normalizeText(bootstrap?.me?.id);
    if (!myUserId) return false;
    return activeMessages.some((message) => {
      if (normalizeText(message.senderAdminUserId) !== myUserId) return false;
      if (normalizeText(message.readByRecipientAt)) return false;
      return true;
    });
  }, [activeConversation, activeMessages, bootstrap?.me?.id]);
  ...
  const showBrowserNotification = useCallback(
    (input: { title: string; body: string; conversationId: string }) => {
      ...
      notification.onclick = () => {
        window.focus();
        if (hideLauncher) {
          const targetPath = `/ops/messenger/${encodeURIComponent(input.conversationId)}`;
    ...
    [browserNotifyEnabled, hideLauncher]
  );
  ...
  const loadBootstrap = useCallback(async () => {
    ...
    try {
      const response = await axios.get<ChatBootstrap>('/api/admin/chat/bootstrap', {
        headers,
        params: { _ts: Date.now(), appKind: 'ops', clientId: chatClientId },
      });
  ...
  useEffect(() => {
    if (!open || !activeConversationId) return;
    if (!activeDirectConversationHasPendingReadReceipts) return;
    ...
    tick();
    const timer = window.setInterval(tick, CHAT_READ_RECEIPT_FAST_REFRESH_MS);
    return () => {
      cancelled = true;
      window.clearInterval(timer);
    };
  }, [open, activeConversationId, activeDirectConversationHasPendingReadReceipts, loadMessages]);
  ...
  return (
    <>
      ...
      {embedded ? (
        <Box
          sx={{
            height: { xs: 'calc(100dvh - 178px)', md: 'calc(100dvh - 208px)' },
      ...
      {callOverlayVisible ? (
        <Paper
          ...
          sx={{
            ...
            py: 0.9,
            minWidth: 220,
            maxWidth: { xs: 'calc(100vw - 20px)', sm: 460 },
